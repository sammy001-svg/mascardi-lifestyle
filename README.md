...
## Status: Phase 4 complete (Events & ticketing)
...
**Phase 4 — Events**: admin **Events** module (paid/free toggle, capacity — leave blank for unlimited, venue, start/end date-time, image) and a **Registrations** module (filter by event, check-in, CSV export). Public: `/events` listing and `/events/{slug}` detail pages, a registration form that branches automatically — free events RSVP instantly with a generated ticket code and row-locked capacity check; paid events reuse the exact same M-Pesa Daraja flow as the shop (waiting/retry/confirmation pages) with capacity checked and locked only once payment actually confirms, so abandoned checkouts never hold a phantom seat. If a paid ticket's payment lands after the event is technically full (money already captured), it's still confirmed and flagged for manual review rather than declining a paying customer — same principle as the Shop's oversold handling. The homepage Events teaser and admin Dashboard now show real upcoming events.

**Not yet live**: real Daraja credentials — `config/mpesa.php` still has placeholder sandbox values, so STK push initiation will fail until real Consumer Key/Secret/Shortcode/Passkey are added (see "M-Pesa go-live" below). Go-live polish (Phase 5: full responsive/security pass, production M-Pesa cutover, cPanel deployment runbook, SEO) is next.

> Note: I validated Phases 2–4 by rendering pages server-side, running full HTTP request flows (admin CRUD round-trips, add-to-cart → checkout → simulated M-Pesa callback → stock decrement, free RSVP capacity-overflow rejection, paid ticket → simulated callback → capacity-locked confirmation, admin check-in), and checking logs/HTML for errors — no PHP warnings anywhere, CSRF protection confirmed on all public POST routes, callback idempotency confirmed on duplicate webhook delivery. I don't have a browser tool in this environment to visually screenshot it, so a look in an actual browser is still worth doing before you consider it final.
