...
<section class="section section--offset-header">
    <div class="container" style="max-width:560px;text-align:center;">
        <span class="section-head__eyebrow"><?= e($event['title']) ?></span>
        <h1 class="section-head__title" style="margin-top:18px;"><?= e(Money::format((int) $registration['total_amount_cents'])) ?></h1>

        <div id="paymentWaiting"
             data-ref="<?= e($transaction['checkout_request_id'] ?? '') ?>"
             data-status-url="/mpesa/status.php"
             data-confirmation-url="<?= site_url('events/confirmation/' . $registration['ticket_code']) ?>"
             style="margin-top:34px;">

            <?php if ($hasActivePush): ?>
                <div data-state="waiting">
                    <p style="font-weight:700;margin-bottom:8px;">Check your phone</p>
                    <p style="color:var(--color-gray-600);font-size:0.92rem;">Enter your M-Pesa PIN on <?= e($registration['attendee_phone']) ?> to complete this payment.</p>
                ...
                <div data-state="failed" style="display:none;">
                    ...
                    <form method="post" action="<?= site_url('events/retry/' . $registration['ticket_code']) ?>">
                        <?= csrf_field() ?>
            ...
            <?php elseif ($hasFailed || $neverStarted): ?>
                ...
                <p style="color:var(--color-gray-600);font-size:0.92rem;margin-bottom:22px;"><?= e($startError ?: "We couldn't send the M-Pesa prompt. Your ticket reservation is saved — you can try again.") ?></p>
                <form method="post" action="<?= site_url('events/retry/' . $registration['ticket_code']) ?>">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-dark">Try Again</button>
                </form>
